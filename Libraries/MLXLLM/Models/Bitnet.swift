...
private class BitLinear: Module {
    ...
    private var compiledKernel: MLXFast.MLXFastKernel?
    
    init(_ inFeatures: Int, _ outFeatures: Int, bias: Bool = true, dtype: DType = .float16, invertWeightScales: Bool = false) {
        self.dtype = dtype
        self.inFeatures = inFeatures
        self.outFeatures = outFeatures
            
        let packedOutFeatures = Int(floor(Double(outFeatures + 3) / 4.0))
        _weight.wrappedValue = MLXArray.zeros([packedOutFeatures, inFeatures], dtype: .uint8)
            
        self.invertWeightScales = invertWeightScales
        _weightScale.wrappedValue = MLXArray([1]).asType(dtype)
            
        if bias {
            _bias.wrappedValue = MLXArray.zeros([outFeatures], dtype: dtype)
        } else {
    ...
    private func bitlinearKernel(_ x: MLXArray, _ packedWeights: MLXArray) -> MLXArray {
        let source = """
        ...
        if compiledKernel == nil {
            compiledKernel = MLXFast.metalKernel(
        ...
        let outputs = compiledKernel!.callAsFunction(
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        ...
        var y = bitlinearKernel(x, weight)
...
private class BitnetAttention: Module {
    ...
    init(_ args: BitnetConfiguration) {
        ...
        rope = RoPE(dimensions: headDim, traditional: args.ropeTraditional, base: args.ropeTheta, scale: ropeScale)
...
private class BitnetMLP: Module {
...
private class BitnetTransformerBlock: Module {
    @ModuleInfo(key: "self_attn") var attention: BitnetAttention
    @ModuleInfo(key: "mlp") var mlp: BitnetMLP
    ...
    init(_ args: BitnetConfiguration) {
        ...
        _mlp.wrappedValue = BitnetMLP(args)
...
private class BitnetModelInner: Module {
    ...
    let layers: [BitnetTransformerBlock]
    ...
    init(_ args: BitnetConfiguration) {
        ...
        layers = (0..<args.hiddenLayers).map { _ in BitnetTransformerBlock(args) }
