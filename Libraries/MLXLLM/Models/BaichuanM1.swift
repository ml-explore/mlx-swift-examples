...
private class Attention: Module {
    ...
    init(_ config: BaichuanM1Configuration, layerIdx: Int) {
        ...
        self.numHeads = isSWA && config.swaAttentionHeads != nil ? config.swaAttentionHeads! : config.attentionHeads
        ...
        _wPack.wrappedValue = Linear(config.hiddenSize, config.hiddenSize + 2 * numKVHeads * headDim, bias: false)
    ...
    func customConvolution(_ u: MLXArray, _ weights: MLXArray, state: MLXArray? = nil) -> MLXArray {
        ...
        let uPrev: MLXArray = L > 1 ? concatenated([state, u[0..., 0..., ..<(L-1), 0...]], axis: 2) : state
...
private class BaichuanM1ModelInner: Module {
    ...
    func callAsFunction(_ inputs: MLXArray, mask: MLXFast.ScaledDotProductAttentionMaskMode? = nil, cache: [KVCache]?) -> MLXArray {
...
public class BaichuanM1Model: Module, LLMModel, KVCacheDimensionProvider {
    ...
    public func newCache(parameters: GenerateParameters?) -> [KVCache] {
        return model.layers.enumerated().map { (i, _) in
            ...
            let kvCache: KVCache = isSWA ? RotatingKVCache(maxSize: configuration.slidingWindow) : KVCacheSimple()
