...
public final class ModelAdapterFactory {
    ...
    public func load(
        ...
        let snapshotURL = try await downloadModel(
            hub: hub, configuration: configuration, progressHandler: progressHandler)
        let configurationURL = snapshotURL.appending(path: "adapter_config").appendingPathExtension(
            "json")
        let configurationData = try Data(contentsOf: configurationURL)
        let configuration = try decoder.decode(BaseConfiguration.self, from: configurationData)

        let weightsURL = snapshotURL.appending(path: "adapters").appendingPathExtension(
            "safetensors")
        ...
        case .lora, .dora:
            return LoRAContainer(
                configuration: try decoder.decode(
                    LoRAContainer.Configuration.self, from: configurationData),
